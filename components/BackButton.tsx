 ...
 * mobile, not just a small arrow glyph. `min-h-[44px]` keeps the tap
 * target at Apple/Google's own minimum recommended size even though the
 * text+icon content is visually smaller than that.
 */
export default function BackButton({ onClick, label = "Vissza" }: { onClick: () => void; label?: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="-ml-2 mb-3 flex min-h-[44px] items-center gap-1.5 rounded-md px-2.5 py-2 text-sm font-medium text-muted transition-colors hover:bg-ivory-dim hover:text-forest"
    >
