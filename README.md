...
| Page | What it does |
|---|---|
| **Áttekintés** (Overview) | Quick stats + a recent activity feed pulled from every table — every item links straight to its page (tasks deep-link into their detail view). An extra **Olvasatlan levelek** stat card appears whenever Gmail is connected |
...
3. Open [`supabase/schema.sql`](./supabase/schema.sql) from this repo,
   ...
   `price-quotes`, `marketing`, `gold-card-letters`, and
   `journey-memories` Storage buckets used for file uploads.
