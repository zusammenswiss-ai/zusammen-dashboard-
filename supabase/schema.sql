...
-- =====================================================================
-- Rituals + Cards — "connection card" content management (Kártyák /
-- Rituálék menu). A Ritual is a named, ordered sequence of steps (built
-- in the Ritual Builder); a Card is one piece of card content (front
-- question, deep question, NFC/QR metadata for the physical product)
-- that can optionally point at a Ritual to walk through. Both share the
-- same draft/testing/published/archived workflow and a version_history
-- log: every save appends the record's *previous* state (tagged with
-- its own version label) to version_history before writing the new
-- values, so "what did v1.0 say" stays answerable after editing to
-- v1.1 — unlike unlock_history above (which only logs the unlock
-- event), this logs a full content snapshot per version. A status-only
-- change (no content edit) appends an entry with snapshot = null.
-- =====================================================================
create table if not exists public.rituals (
  ...
  -- Ordered list of step strings — array order *is* step order, no
  -- separate step-number field needed (see components/RitualStepsEditor.tsx).
...
drop policy if exists "authenticated full access" on public.rituals;
create policy "authenticated full access" on public.rituals for all
  using (auth.uid() is not null) with check (auth.uid() is not null);

drop trigger if exists set_updated_at on public.rituals;
create trigger set_updated_at before update on public.rituals
  for each row execute function public.set_updated_at();

create table if not exists public.cards (
  id uuid primary key default gen_random_uuid(),
  -- Human-friendly sequential number shown as "Card #07" — separate
  -- from the uuid primary key, assigned once on insert, never reused.
  card_number integer generated by default as identity,
  title text not null,
  category text,
  question text,
...
drop policy if exists "authenticated full access" on public.cards;
create policy "authenticated full access" on public.cards for all
  using (auth.uid() is not null) with check (auth.uid() is not null);
